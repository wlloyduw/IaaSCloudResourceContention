...
def pssh(minute='*', hour='*', day='*', cycles='1', benchmark='pgbench'):
	...
	task=\\'$_task\\'
	psshcommand='set -f && eval "$(ssh-agent -s)" && ssh-add -k ~/.ssh/git_capstone && rm -rf Capstone SCRIPT && git clone https://github.com/wlloyduw/IaaSCloudResourceContention.git && mv Capstone SCRIPT && cd ~/SCRIPT && cp /etc/crontab . && echo '$task' >> crontab && sudo mv crontab /etc/crontab && sudo chown root.root /etc/crontab && sudo service cron reload'
...
def pssh_v2(target_time=datetime.datetime.utcnow()+relativedelta(minutes=5), cycles='10', interval=15, benchmark='pgbench', reverseFlag=False, vmgen='c3'):
    ...
    def getPsshcommand(minute, hour, day, HOST_STRING):
        return '''
		set -f
		psshcommand='set -f && echo "''' + minute + " " + hour + " " + day + ''' * * ubuntu python3  ~/SCRIPT/scripts/remote/run.py -c ''' + cycles+' -t '+benchmark + ' -i ' + exp_id + ' | logger -t testharness' + '''" >> crontab'
...
def cloneGitRepo():
	...
	git clone https://github.com/wlloyduw/IaaSCloudResourceContention.git && mv Capstone SCRIPT'
